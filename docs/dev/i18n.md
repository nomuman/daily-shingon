...
- 端末ロケール取得は Expo 公式の `expo-localization` を使う。
- UI文言の翻訳管理は i18next / react-i18next を使う（React Native対応・エコシステムが強い）。

> Expo公式ガイドでは `expo-localization` + `i18n-js` の例もあるが、言語数/文量が増えるなら i18next の伸びが良い。
...
- i18next の plural は `count` が必須（`count` が無いと期待通りに出ない）。

5. **RTL（右→左）を壊さない**

- 必要に応じて `I18nManager` で RTL を扱う。
...
- `expo-localization` の `getLocales()` を使う。

### Androidの注意（超重要）

Androidは、ユーザーが設定で言語を変えても **アプリ再起動なしで変わる**ことがある。
そのため、**フォアグラウンド復帰時に `getLocales()` を取り直す**（AppStateなどで検知）。
...
- 通常は `t('key.path')` で十分
- React要素を埋め込みたい時だけ `<Trans>` を使う（使いすぎない）

### 4.2 Plural（複数形）ルール

- i18next の plural は `count` が必須。
...
- React Native は RTL 向けに `I18nManager` を提供している。
- 基本は Flexbox を正しく使っていれば多くのUIは対応できるが、以下は要注意:
  - 絶対配置（`position: 'absolute'`）で left/right に依存
  - アニメーションで x方向の移動が固定
- 対処:
  - `I18nManager.isRTL` を見て調整（必要な場所だけ）

---

## 7. ストアで「対応言語」をOS設定に出す（任意だが推奨）

よりOSネイティブな言語設定表示（Android 13+ / iOS）に寄せたい場合、`react-native-localize` の Expo plugin で「対応言語」を宣言できる。
